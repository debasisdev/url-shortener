...
/**
 * Test placeholder to test URL Service {@link UrlService}.
 * 
...
public class UrlServiceTest {
	...
	public void givenUrl_a_checkValidity() {
		...
		List<Boolean> validities = Arrays.asList(sampleUrls).stream().map(url -> urlService.isUrl(url))
	...
	public void givenShortUrl_getHash() {
		...
		List<String> hashList = Arrays.asList(sampleUrls).stream().map(url -> urlService.getHash(url))
		        .collect(Collectors.toList());
	...
	public void givenUrl_getShortUrl() {
		...
		List<String> shortUrlList = urlList.stream().map(url -> urlService.shortenUrl(url).getPath())
		        .collect(Collectors.toList());
	...
	public void givenShortUrl_getDatabaseId() {
		String[] shortUrls = { "http://goog.le/bkXSbO", "https://wikiped.ia/bkXSbO", "http://stackoverfl.ow/bkXSbO",
		        "ftp://we.io/bkXSbO", "http://spoti.fy/bkXSbO", "http://pl.ay/bkXSbO" };
		Set<Long> shortUrlSet = Arrays.asList(shortUrls).stream().map(url -> urlService.getDatabaseId(url))
